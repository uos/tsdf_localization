...
std::shared_ptr<MapT> createIntTSDFMap(std::string h5_filename)
{
    ...
    std::vector<int_tsdf_localization::FIXED_T> max(3, 0);
    ...
    for (auto index = 0u; index < min.size(); ++index)
    {
        min[index] = min[index] * CHUNK_SIZE * MAP_RESOLUTION * (int_tsdf_localization::INT_RESOLUTION / 1000);
        max[index] = (max[index] * CHUNK_SIZE + CHUNK_SIZE) * MAP_RESOLUTION * (int_tsdf_localization::INT_RESOLUTION / 1000);
    }

    auto map = std::make_shared<MapT>(MapT(min[0], min[1], min[2], max[0], max[1], max[2], MAP_RESOLUTION * (int_tsdf_localization::INT_RESOLUTION / 1000), -1000));
        ...
        for (int i = 0; i < CHUNK_SIZE; i++)
        {
            for (int j = 0; j < CHUNK_SIZE; j++)
            {
                for (int k = 0; k < CHUNK_SIZE; k++)
                {
                    ...
                    auto tsdf_value = (int_tsdf_localization::FIXED_T)(entry.value());
                    auto weight = entry.weight();
                    ...
                    if (weight != 0 && std::abs(tsdf_value) < truncation)
                    {
                        auto pos_x = static_cast<int_tsdf_localization::FIXED_T>(x) * MAP_RESOLUTION * (int_tsdf_localization::INT_RESOLUTION / 1000);
                        auto pos_y = static_cast<int_tsdf_localization::FIXED_T>(y) * MAP_RESOLUTION * (int_tsdf_localization::INT_RESOLUTION / 1000);
                        auto pos_z = static_cast<int_tsdf_localization::FIXED_T>(z) * MAP_RESOLUTION * (int_tsdf_localization::INT_RESOLUTION / 1000);

                        //map->setEntry(pos_x, pos_y, pos_z, tsdf_value * 0.001);

                        data.push_back(std::make_tuple(pos_x, pos_y, pos_z, tsdf_value * (int_tsdf_localization::INT_RESOLUTION / 1000)));
